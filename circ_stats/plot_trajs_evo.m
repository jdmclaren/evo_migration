...
elseif mig_sys == 2
    n_Scan_0s = round(N_plt/10); % N_plt; % 
    idx_Scans = find((blats(:,1) > 62.5*pi/180),n_Scan_0s,'first'); % 1:N_plt; % find((blats(:,1) > 55*pi/180) & ...
%     (blons(:,1) > 15*pi/180 & ...
%     blons(:,1) < 30*pi/180) & ...
%     abs(allel_heads(:,1) - allel_heads(:,2)) > pi/9, ...
%     n_Scan_0s,'first');
    plt_ord = [1:N_plt idx_Scans'];

    is_div = abs(allel_heads(:,1) - allel_heads(:,2)) > pi/9 & ...
        sign(allel_heads(:,1).*allel_heads(:,2)) < 0;

    disp(num2str(round(100*nansum(stoppedAndArrived(is_div))/sum(is_div))/100))
    ...
    if n_alles > 1
         ...
         M1s = all_allels(:,1,2);
         M2s = all_allels(:,2,2);   
        min_dv_hd = min(allel_heads(is_div,:),[],2);
        max_dv_hd = max(allel_heads(is_div,:),[],2);
        avgd = (M1s.*M2s==1) | (M1s == M2s & T1s == T2s);
    end
%    plt_ord = find(T1s+T2s==1,n_Scan_0s,'first'); % M1s+M2s>0 ~avgd

%         all_allels(:,1,1) == all_allels(:,2,1) & ...
%     all_allels(:,1,2) == all_allels(:,2,2), ...
...
%     figure
%     scatter(min_dv_hd*180/pi,max_dv_hd*180/pi,60, ...
%     avgd(is_div),'fill')
%     cb = colorbar;
%     title(cb,'averaged headings')

    n_div = sum(is_div);

    TTMM = 100*sum(T1s(is_div).*T2s(is_div) == 1 & M1s(is_div).*M2s(is_div) == 1)/n_div;
    TtMm = 100*sum(T1s(is_div)~=T2s(is_div) & M1s(is_div)~=M2s(is_div))/n_div;
    TTMm = 100*sum(T1s(is_div).*T2s(is_div) == 1 & M1s(is_div)~=M2s(is_div))/n_div;
    TtMM = 100*sum(T1s(is_div)~=T2s(is_div) & M1s(is_div).*M2s(is_div) == 1)/n_div;
    TTmm = 100*sum(T1s(is_div).*T2s(is_div) == 1 & M1s(is_div)+M2s(is_div) == 0)/n_div;
    Ttmm = 100*sum(T1s(is_div)~=T2s(is_div) & M1s(is_div)+M2s(is_div) == 0)/n_div;
    ttMM = 100*sum(T1s(is_div)+T2s(is_div) == 0 & M1s(is_div).*M2s(is_div) == 1)/n_div;
    ttMm = 100*sum(T1s(is_div)+T2s(is_div) == 0 & M1s(is_div)~=M2s(is_div))/n_div;
    ttmm = 100*sum(T1s(is_div)+T2s(is_div) == 0 & M1s(is_div)+M2s(is_div) == 0)/n_div;
    
%     sum(TTMM + TtMm + TTMm + TtMM + TTmm + Ttmm + ttmm + ttMm + ttMM)
...
%         lons_intp = interp1(stps_i,blons_i,substps_i);
     ...
     if plot_opt == 1
         plot_var = hds_intp; % (M1s(ii)+M2s(ii))*ones(size(lats_intp)); % 
